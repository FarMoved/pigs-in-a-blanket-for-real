...
- Assign:
  ...
  - **Kill Feed:** Optional — if left empty, HUDController auto-creates a top-right kill feed container and entry template at runtime.
...
## In-match settings overlay (ESC)

- Add **SettingsOverlayController** to the **Canvas** (replaces the old PauseMenuController).
- UI is built at runtime by **SettingsOverlayUIBuilder** (Valorant-style translucent overlay).
- **ESC** opens/closes settings; game does **not** pause (`Time.timeScale` unchanged).
- **Top-right LEAVE** opens a modal with **LEAVE MATCH** → `NetworkManager.LeaveRoom()`.
- Settings persist via **GameSettings** (PlayerPrefs): mouse sensitivity, volume, invert Y, kill feed toggle.
- The legacy **PauseMenuPanel** in the scene is hidden automatically if still present.
